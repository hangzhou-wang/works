	...
	$.fn.zySearch = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				html += '<input id="searchInput" class="search-input" type="text" placeholder="请输入用户名">';
			...
			this.addEvent = function(){
				...
				document.onkeypress=function(event){
		            ...
		            if(e && e.keyCode==13){ // enter 键
		            	if ($.trim(userName).length==0) {
			        		top.$.messager.alert('提示','请输入要查询的用户名'); 
							//alert("请输入要查询的用户名");
							return false;
						}
		            	// 回调方法
		        ...
		        $(".search-btn").bind("click",function(){
		        	...
		        	//location.href="getUserByName.action?user_name="+$.trim(userName);
		        	//alert(userName);
		        	// 回调方法
	            	para.callback($("#searchInput").val()); 
				});
